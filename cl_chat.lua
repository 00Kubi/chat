...
-- client callout chat role
-- RegisterNetEvent('chat:client:SendLocalMessage')
-- AddEventHandler('chat:client:SendLocalMessage', function(playerId, playerName, message)
--     QBCore.Functions.TriggerCallback('chat:server:getPlayerRole', function(role)
--         local playerServerId = GetPlayerServerId(PlayerId())
--         local currentTime = GetClockHours() .. ':' .. (GetClockMinutes() < 10 and '0' .. GetClockMinutes() or GetClockMinutes())
--         local formattedMessage = string.format('<div class="chat-message"><span class="chat-role">%s</span><span class="chat-name">%s</span><span class="chat-id">%s</span><span class="chat-time">%s</span><br><span class="chat-message-text">%s</span></div>', role, playerName, playerServerId, currentTime, message)
